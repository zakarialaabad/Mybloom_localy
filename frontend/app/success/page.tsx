...
export default function OrderSuccessPage() {
  ...
  // ce qui emporte le header vers le haut. On le neutralise ici.
  useEffect(() => {
  ...
  return (
    <div className="md:block font-serif bg-white">
      ...
      {/* ════════════════════════════════════
          ...
          Stratégie : fixed inset-0
          → colle exactement au viewport, ignore le scroll du body
          → le header (shrink-0) reste TOUJOURS visible en haut
          → seul le div flex-1 overflow-y-auto défile à l'intérieur
      ════════════════════════════════════ */}
      <div className="md:hidden fixed inset-0 flex flex-col overflow-hidden bg-white z-40">

        {/* ── Header mobile – ne peut pas disparaître ── */}
        ...
        {/* ── Zone scrollable — fond rose limité ici ── */}
        <div
          ...
          <div className="flex flex-col justify-center min-h-full px-4 py-6 pb-[72px]">
            <div className="w-full bg-white rounded-xl shadow-[0_2px_18px_rgba(0,0,0,0.07)] px-6 py-7">
              ...
              <div className="flex justify-center mb-6">
                <div className="w-[56px] h-[56px] relative flex items-center justify-center shrink-0">
                  ...
                  <div className="relative z-10 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-sm">
                    <Check className="w-[18px] h-[18px] text-[#da2966]" strokeWidth={3} />
                  </div>
                </div>
              </div>
              ...
              <div className="text-center mb-6">
                <h1 className="text-[17px] font-bold text-gray-800 mb-2 tracking-wide uppercase">COMMANDE REÇUE</h1>
                <p className="text-gray-500 text-[12.5px] leading-relaxed">
                  Merci pour votre achat. Votre voyage parfumé commence maintenant, journey begins now.
                </p>
              </div>

              {/* Order Info */}
              <div className="space-y-4 border-t border-gray-100 pt-5 pb-4 mb-4">
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-[11px] tracking-wide uppercase">NUMÉRO DE COMMANDE</span>
                  <span className="font-bold text-gray-900 text-[12px]">#{order || 'LX-8921-Q'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-[11px] tracking-wide uppercase">DATE</span>
                  <span className="font-bold text-gray-900 text-[12px]">
                    {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </span>
                </div>
                <div className="flex justify-between items-start">
                  <span className="text-gray-400 text-[11px] tracking-wide uppercase mt-0.5">LIVRAISON À</span>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                  <span className="font-bold text-gray-900 text-[12px] tracking-wide uppercase">MONTANT TOTAL</span>
                  <span className="font-bold text-gray-900 text-[16px]">
                    {total ? `${Number(total).toFixed(2)} DH` : '720.00 DH'}
                  </span>
                </div>
              </div>

              {/* Confirmation Call Box */}
              <div className="bg-[#fff5f7] rounded-lg px-4 py-4 mb-5">
                <div className="flex items-center gap-2 mb-2">
                  <Headphones className="w-[13px] h-[13px] text-[#da2966] shrink-0" />
                  <span className="text-[#da2966] font-bold text-[14px] tracking-widest uppercase leading-none">
                    APPEL DE CONFIRMATION
                  </span>
                </div>
                <p className="text-gray-500 text-[11.5px] leading-relaxed">
                  Notre concierge vous contactera bientôt à{' '}
              ...
              <div className="flex gap-3 mb-5">
                <Link
                  href="/"
                  className="flex-1 bg-[#4b3d37] text-white py-3.5 rounded-[4px] italic text-[12.5px] hover:bg-[#3a322d] transition-colors text-center shadow-md font-bold"
                >
                  Acheter maintenant ›
                </Link>
                <Link
                  href={`/order-status?order=${encodeURIComponent(order)}&phone=${encodeURIComponent(phone)}`}
                  className="flex-1 bg-white text-[#4a403a] border border-gray-200 py-3.5 rounded-[4px] italic text-[12.5px] hover:bg-gray-50 transition-colors text-center font-bold shadow-sm"
                >
                  Suivre ma commande
                </Link>
              </div>
        ...
        <div className="w-1/2 flex flex-col justify-center items-center p-8 lg:p-12">
          <div className="w-full max-w-[480px] bg-white shadow-[0_4px_24px_rgba(0,0,0,0.04)] rounded-lg p-10 lg:p-12">
            ...
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold text-gray-800 mb-2 tracking-wide uppercase">COMMANDE REÇUE</h1>
            ...
            {/* Order Info */}
            <div className="space-y-5 border-t border-gray-100 py-6 mb-2">
              ...
              <div className="flex justify-between items-center text-[13px] pt-4 border-t border-gray-50">
            ...
            <div className="bg-[#fff5f7] rounded-lg p-5 mb-8">
              <div className="flex items-center gap-2 mb-2">
                <Headphones className="w-4 h-4 text-[#da2966] shrink-0" />
                <span className="text-[#da2966] font-bold text-[14px] tracking-widest uppercase">
                  APPEL DE CONFIRMATION
                </span>
              </div>
