...
export default function CollectionPage() {
  ...
  useEffect(() => {
    if (brands.length > 0 && categories.length > 0 && ingredients.length > 0) setPageLoading(false);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const searchTerm  = searchParams.get('search');      if (searchTerm)  params['search']        = searchTerm;
      const productType = searchParams.get('product_type'); if (productType) params['product_type']  = productType;
      const catSlug     = searchParams.get('cat');          if (catSlug)     params['category']      = catSlug;
      ...
      if (featuredOnly) {
        ...
        if (aggregatesReady) { params['price_min'] = selectedMin; params['price_max'] = selectedMax; }
      ...
      try {
        ...
        if (err instanceof Error && err.name !== 'AbortError' && err.name !== 'CanceledError') setProducts([]);
      } finally {
        setLoadingProducts(false);
      }
    }, 400);
    return () => { clearTimeout(timer); controller.abort(); };
  }, [searchParams, selectedBrands, selectedCategories, selectedIngredients, selectedMin, selectedMax, selectedRating, promotionOnly, featuredOnly, aggregatesReady, sortBy, ensureProductsCache]);
